class Ahoy::Visit < ApplicationRecord
  ...
  scope :legitimate, -> { Rails.env.production? ? where("user_id IS NOT NULL OR (duration > 0 AND (referrer_host IS NULL OR referrer_host != '#{ENV["HOST_NAME"]}'))") : where("duration > 0") }
